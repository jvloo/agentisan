...
mod unix {
    ...
    pub async fn run_with_host(
        ...
        let outcome=async {
            ...
            let prompt=format!("{prompt}\nDelivery protocol: inbox_read only reads the inputs claimed for this turn. message_send stages outgoing messages. After processing all inputs, call turn_commit with this run_id and a unique idempotency_key before ending the turn. turn_commit atomically acknowledges those inputs and publishes your staged messages. A successful result_propose already commits the lead's inputs and proposal; do not call turn_commit afterward. Never claim a message was delivered before commit succeeds. A failed proposal does not commit: call turn_commit then end your turn.");
