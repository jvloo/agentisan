...
mod unix {
    ...
    pub async fn run_with_host(
        ...
        let outcome=async {
            ...
            let prompt=format!("{prompt}\nDelivery protocol: inbox_read presents the stable inputs and current work records claimed for this turn without acknowledging them. The lead should use assignment_create for bounded delegated work; an assignee uses assignment_update to report it, and the lead closes reported assignments before result_propose. Use decision_request only for a concrete human choice bound to exact scope and artifact hashes; it never grants approval. message_send stages peer discussion. All these effects stay private until turn_commit atomically acknowledges inputs and publishes them. A successful result_propose already commits the lead's inputs, work updates, and proposal; do not call turn_commit afterward. Never claim delivery, assignment progress, or approval before commit succeeds. A failed proposal does not commit: call turn_commit then end your turn.");
